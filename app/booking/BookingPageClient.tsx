...
export default function BookingPageClient({ categories }: { categories: BookingCategory[] }) {
    ...
    return (
        <>
            <Navbar />
            <section className="relative overflow-hidden bg-[#F6F5F1] py-24 md:py-32 text-neutral-900 dark:bg-neutral-900 dark:text-white">
            ...
            <section className="bg-[#F6F5F1] pb-24 md:pb-32 dark:bg-neutral-900">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
                    <div className="divide-y divide-neutral-200/60 dark:divide-neutral-700">
                        {categories.map((category) => (
                            <div
                                key={category.slug}
                                className="group flex items-center justify-between py-8 md:py-10 transition-all duration-300 hover:bg-white/50 dark:hover:bg-neutral-800/50"
                            >
                                ...
                                >
                                    <Link href={`/booking/${category.slug}`}>Select</Link>
                                </Button>
                            </div>
                        ))}
                    </div>
