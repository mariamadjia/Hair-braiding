        ...
        <div className="space-y-4">
            ...
            <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                    {items.length > 0 && <section aria-labelledby="bulk-foundation-title" className="overflow-hidden rounded-xl border border-[#E7DDD7] bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                        <div className="flex flex-col gap-2 border-b border-[#EEE6E1] bg-[#FBF8F5] px-5 py-4 sm:flex-row sm:items-start sm:justify-between dark:border-neutral-700 dark:bg-neutral-800/70">
                            <div><div className="flex flex-wrap items-center gap-2"><h4 id="bulk-foundation-title" className="text-sm font-semibold text-neutral-900 dark:text-white">Braid foundation for all sizes</h4><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-[#6B493D] ring-1 ring-[#E4D6CF] dark:bg-neutral-900">{items.length} sizes</span></div><p className="mt-1 text-xs leading-5 text-neutral-500">Set the customer’s foundation choices once for every size in {sub.name}.</p></div>
                            <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-neutral-400">Bulk edit</span>
                        </div>

                        <div className="space-y-5 p-5">
                            <fieldset><legend className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-neutral-500">Customer options</legend><div className="grid gap-3 sm:grid-cols-2">
                                <button type="button" aria-pressed={!bulkFoundationEnabled} onClick={() => setBulkFoundationEnabled(false)} className={`flex min-h-20 items-start gap-3 rounded-lg border p-4 text-left transition focus:outline-none focus:ring-2 focus:ring-[#2C1810] focus:ring-offset-2 ${!bulkFoundationEnabled ? "border-[#2C1810] bg-[#FAF7F2] shadow-sm" : "border-neutral-200 hover:border-neutral-400 dark:border-neutral-700"}`}><span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${!bulkFoundationEnabled ? "border-[#2C1810] bg-[#2C1810] text-white" : "border-neutral-300"}`}>{!bulkFoundationEnabled && <CheckCircle className="h-3.5 w-3.5" />}</span><span><span className="block text-sm font-semibold">No foundation choice</span><span className="mt-1 block text-xs text-neutral-500">Customers go directly to length selection.</span></span></button>
                                <button type="button" aria-pressed={bulkFoundationEnabled} onClick={() => setBulkFoundationEnabled(true)} className={`flex min-h-20 items-start gap-3 rounded-lg border p-4 text-left transition focus:outline-none focus:ring-2 focus:ring-[#2C1810] focus:ring-offset-2 ${bulkFoundationEnabled ? "border-[#2C1810] bg-[#FAF7F2] shadow-sm" : "border-neutral-200 hover:border-neutral-400 dark:border-neutral-700"}`}><span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${bulkFoundationEnabled ? "border-[#2C1810] bg-[#2C1810] text-white" : "border-neutral-300"}`}>{bulkFoundationEnabled && <CheckCircle className="h-3.5 w-3.5" />}</span><span><span className="block text-sm font-semibold">Regular and Knotless</span><span className="mt-1 block text-xs text-neutral-500">Customers choose a foundation before length.</span></span></button>
                            </div></fieldset>

                            {bulkFoundationEnabled && <div className="grid gap-4 rounded-lg border border-[#EEE6E1] bg-[#FBF8F5] p-4 sm:grid-cols-[minmax(0,1fr)_10rem] sm:items-center dark:border-neutral-700 dark:bg-neutral-800/60"><div><p className="text-sm font-semibold text-neutral-900 dark:text-white">Knotless price adjustment</p><p className="mt-1 text-xs leading-5 text-neutral-500">This amount is added to every existing length price.</p></div><label><span className="sr-only">Knotless adjustment for all sizes</span><span className="flex min-h-12 items-center rounded-lg border border-neutral-300 bg-white shadow-sm focus-within:border-[#2C1810] focus-within:ring-2 focus-within:ring-[#2C1810]/20 dark:border-neutral-600 dark:bg-neutral-900"><span className="border-r border-neutral-200 px-3 text-sm font-semibold text-neutral-500 dark:border-neutral-700">$</span><input aria-label="Knotless adjustment for all sizes" inputMode="decimal" value={bulkFoundationAdjustment} onChange={event => setBulkFoundationAdjustment(event.target.value.replace(/[^0-9.]/g, ""))} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-base font-semibold outline-none" /></span></label></div>}

                            <div className="flex flex-col gap-3 border-t border-neutral-100 pt-4 sm:flex-row sm:items-center sm:justify-between dark:border-neutral-800"><p className="text-xs text-neutral-500">{bulkFoundationEnabled ? <>All {items.length} sizes will offer Regular pricing and Knotless at <strong className="text-neutral-800 dark:text-neutral-200">+${bulkFoundationAdjustment || "0"}</strong>.</> : <>Foundation selection will be removed from all {items.length} sizes.</>}</p><button type="button" onClick={() => void applyFoundationToAllSizes()} disabled={applyingFoundations || saving || !items.some(item => item.id)} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-[#2C1810] px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1a0f0a] focus:outline-none focus:ring-2 focus:ring-[#2C1810] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">{applyingFoundations && <Loader2 className="h-4 w-4 animate-spin" />}{applyingFoundations ? "Applying to all sizes…" : `Apply to all ${items.length} sizes`}</button></div>
                        </div>
                    </section>}
                    {addingItem && (
