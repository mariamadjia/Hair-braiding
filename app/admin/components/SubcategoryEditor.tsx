        ...
        <div className="w-full space-y-7 px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
            ...
            {guideSettings && editingGuide && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/45 p-2 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="guide-editor-title" onMouseDown={event => { if (event.target === event.currentTarget) cancelGuideEditor(); }}>
                <div className="max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto rounded-t-2xl bg-white shadow-2xl dark:bg-neutral-900 sm:max-h-[90vh] sm:rounded-2xl">
                    ...
                    <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t border-[#e4d8cc] bg-[#fffdf9]/95 px-5 py-4 shadow-[0_-10px_28px_rgba(57,32,18,0.08)] backdrop-blur dark:border-neutral-700 dark:bg-neutral-900/95 sm:px-6"><p className="text-xs font-medium text-neutral-500">{guidesDirty ? "Unsaved changes" : "No changes to save"}</p><div className="flex gap-3"><button type="button" onClick={cancelGuideEditor} className="min-h-10 rounded-xl border border-[#d9cabd] bg-white px-5 text-sm font-semibold text-[#351a10] dark:border-neutral-600 dark:bg-neutral-900 dark:text-white">Discard</button><button type="button" onClick={() => void saveGuideSettings(guideSettings, true)} disabled={!guidesDirty || savingGuides || uploadingGuide !== null} className="min-h-10 rounded-xl bg-[#351a10] px-5 text-sm font-semibold text-white disabled:opacity-40 dark:bg-white dark:text-neutral-950">{savingGuides ? "Saving…" : "Save changes"}</button></div></div>
                </div>
            </div>}
