...
interface Appointment {
    id: number; serviceName: string; selectedSize: string | null; selectedLength: string | null; appointmentDateTime: string; appointmentEndDateTime: string | null;
    durationMinutes: number | null; status: string; paymentStatus: string | null; amountPaid: number; paymentOutcome: string;
    cancelledByCustomer: boolean | null; cancellationReason: string | null;
}
...
function CustomerDetails({ customerId, onBack }: { customerId: number; onBack: () => void }) {
    ...
    const fetchDetails = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/customers/${customerId}?${params}`, { credentials: "include", cache: "no-store", signal });
            ...
            setCustomer(body);
        } catch (err) { if (!(err instanceof DOMException && err.name === "AbortError")) setError(err instanceof Error ? err.message : "Failed to load customer details"); }
        finally { if (!signal?.aborted) setLoading(false); }
    }, [customerId, page, status]);

    useEffect(() => { const controller = new AbortController(); void fetchDetails(controller.signal); return () => controller.abort(); }, [fetchDetails]);

    const date = (value: string | null, withTime = false) => { if (!value) return "—"; const parsed = new Date(value.endsWith("Z") ? value : `${value}Z`); if (Number.isNaN(parsed.getTime())) return "Date unavailable"; const formatted = parsed.toLocaleString("en-US", withTime ? { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit", timeZone: "UTC" } : { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }); return withTime ? `${formatted} CT` : formatted; };
    const money = (value: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(value);
    const statusClass = (value: string) => ({ APPROVED: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200", PENDING: "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200", APPROVAL_PENDING_CAPTURE: "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200", DENIED: "bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-200", CANCELLED: "bg-neutral-200 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-200", COMPLETED: "bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-200", NO_SHOW: "bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-200" }[value] ?? "bg-neutral-100 text-neutral-700");
    const paymentLabel = (appointment: Appointment) => { switch (appointment.paymentOutcome) { case "DEPOSIT_RETAINED": return `Deposit retained: ${money(appointment.amountPaid)}`; case "AUTHORIZATION_RELEASED": return "Authorization released — $0 charged"; case "RELEASE_FAILED": return "Authorization release needs attention"; case "DEPOSIT_CAPTURED": return `Deposit captured: ${money(appointment.amountPaid)}`; case "AUTHORIZED_NOT_CHARGED": return "Authorized — not charged"; default: return `Paid: ${money(appointment.amountPaid)}`; } };
    ...
    return <section className="mx-auto max-w-6xl space-y-6" aria-labelledby="customer-name">
        ...
        {error && <div role="alert" className="rounded-sm border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-900 dark:bg-red-950 dark:text-red-200"><strong>Showing the last loaded customer data.</strong> {error}</div>}
