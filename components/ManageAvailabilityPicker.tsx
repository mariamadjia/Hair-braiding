...
export default function ManageAvailabilityPicker({ token, serviceId, lengthOptionId, currentAppointmentDateTime, busy, onClose, onConfirm, title = "Choose a new date and time", confirmLabel = "Confirm one-time reschedule" }: {
  ...
  return <section className="overflow-hidden rounded-xl border border-[#e2d4c5] bg-white">
    ...
    {step === "date" && <div className="p-5 sm:p-7">
      <div className="mb-5 flex items-center justify-between"><button aria-label="Previous month" disabled={!canGoPrevious} onClick={() => setMonth(value => new Date(value.getFullYear(), value.getMonth() - 1, 1))} className="rounded-lg p-2 hover:bg-[#f8efe7] disabled:opacity-25"><ChevronLeft/></button><h3 className="font-serif text-2xl">{MONTHS[month.getMonth()]} {month.getFullYear()}</h3><button aria-label="Next month" onClick={() => setMonth(value => new Date(value.getFullYear(), value.getMonth() + 1, 1))} className="rounded-lg p-2 hover:bg-[#f8efe7]"><ChevronRight/></button></div>
      <div className="grid grid-cols-7 gap-1.5 sm:gap-2">{DAYS.map(day => <div key={day} className="py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-[#76675e]">{day}</div>)}{days.map((day, index) => {
        ...
        return <button key={key} disabled={!enabled} onClick={() => void selectDate(key)} aria-label={`${MONTHS[month.getMonth()]} ${day}${enabled ? ", appointments available" : ", unavailable"}`} className={`relative aspect-square min-h-10 rounded-lg border text-sm transition ${enabled ? "border-[#ddc9b8] bg-[#fffdf9] hover:border-[#9c5c3b] hover:bg-[#f8efe7]" : "border-[#f0e9e2] bg-[#fbf8f4] text-stone-300"}`}>{day}{status === "loading" && <span className="absolute bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 animate-pulse rounded-full bg-stone-300"/>}{enabled && <span className="absolute bottom-1.5 left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-[#b7602e]"/>}</button>;
      })}</div>
      <div className="mt-5 flex gap-5 border-t border-[#eee3da] pt-4 text-xs text-[#76675e]"><span><i className="mr-2 inline-block h-2 w-2 rounded-full bg-[#b7602e]"/>Available</span><span><i className="mr-2 inline-block h-2 w-2 rounded-full bg-[#ded7d0]"/>Closed or fully booked</span></div>
    </div>}
