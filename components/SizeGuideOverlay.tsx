...
export default function SizeGuideOverlay({ profile, onClose, onShowLength }: { profile: SizeGuideProfile; onClose: () => void; onShowLength?: () => void }) {
  return <div className="fixed inset-0 z-[61] flex flex-col bg-white text-neutral-900 lg:absolute lg:inset-auto lg:left-[calc(100%+1rem)] lg:top-0 lg:h-full lg:w-[32rem] lg:overflow-hidden lg:rounded-xl lg:border lg:border-neutral-200 lg:shadow-[0_20px_60px_rgb(0,0,0,0.3)]" role="dialog" aria-modal="true" aria-labelledby="size-guide-title">
    ...
    {onShowLength && <nav className="grid shrink-0 grid-cols-2 border-b border-neutral-200 bg-white p-2" aria-label="Guide type">
      <button type="button" aria-current="page" className="rounded-lg bg-[#2C1810] px-3 py-2.5 text-xs font-semibold text-white">Size guide</button>
      <button type="button" onClick={onShowLength} className="rounded-lg px-3 py-2.5 text-xs font-semibold text-neutral-600 hover:bg-neutral-100">Length guide</button>
    </nav>}
