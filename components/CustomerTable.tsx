...
function CustomerTable({ onViewDetails, state, onStateChange }: Props) {
    ...
    const CustomerCard = ({ customer }: { customer: Customer }) => <article className="rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-800">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-neutral-100 pt-3 text-sm dark:border-neutral-700"><div><dt className="text-neutral-500">Next appointment</dt><dd className="font-medium text-neutral-900 dark:text-white">{date(customer.nextAppointmentDate)}</dd></div><div><dt className="text-neutral-500">Last completed visit</dt><dd className="font-medium text-neutral-900 dark:text-white">{date(customer.lastAppointmentDate)}</dd></div><div><dt className="text-neutral-500">Total bookings</dt><dd className="font-medium text-neutral-900 dark:text-white">{customer.totalAppointments}</dd></div><div><dt className="text-neutral-500">Completed visits</dt><dd className="font-medium text-neutral-900 dark:text-white">{customer.completedVisits}</dd></div><div className="col-span-2"><dt className="text-neutral-500">Captured deposits</dt><dd className="font-medium text-neutral-900 dark:text-white">{money(customer.totalSpent)}</dd></div></dl>
    </article>;
    ...
    return <section className="mx-auto max-w-7xl space-y-5" aria-labelledby="customers-title">
        ...
        {loading ? <div role="status" className="flex justify-center gap-2 py-16 text-neutral-500"><Loader2 className="h-6 w-6 animate-spin" />Loading customers…</div> : !error && customers.length === 0 ? <div className="py-16 text-center text-neutral-500"><Users className="mx-auto mb-3 h-10 w-10 text-neutral-300" /><p>No customers match these filters.</p></div> : !error && <>
            ...
            <div className="hidden overflow-x-auto rounded-lg border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800 md:block"><table className="w-full min-w-[1120px]"><thead><tr className="border-b border-neutral-200 bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900"><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Next appointment</th><th className="px-4 py-3">Last completed visit</th><th className="px-4 py-3">Total bookings</th><th className="px-4 py-3">Completed visits</th><th className="px-4 py-3">Captured deposits</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">{customers.map(customer => <tr key={customer.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50"><td className="px-4 py-3 font-medium text-neutral-900 dark:text-white">{customer.firstName} {customer.lastName}</td><td className="px-4 py-3 text-sm"><a href={`mailto:${customer.email}`} className="block text-neutral-700 hover:underline dark:text-neutral-200">{customer.email}</a><a href={`tel:${customer.phoneNumber}`} className="block text-neutral-500 hover:underline dark:text-neutral-400">{customer.phoneNumber}</a></td><td className="px-4 py-3 text-sm text-neutral-700 dark:text-neutral-200">{date(customer.nextAppointmentDate)}</td><td className="px-4 py-3 text-sm text-neutral-700 dark:text-neutral-200">{date(customer.lastAppointmentDate)}</td><td className="px-4 py-3 text-sm text-neutral-700 dark:text-neutral-200">{customer.totalAppointments}</td><td className="px-4 py-3 text-sm text-neutral-700 dark:text-neutral-200">{customer.completedVisits}</td><td className="px-4 py-3 text-sm font-medium text-neutral-900 dark:text-white">{money(customer.totalSpent)}</td><td className="px-4 py-3"><Button variant="outline" size="sm" onClick={() => onViewDetails(customer.id)}>View</Button></td></tr>)}</tbody></table></div>
        </>}
