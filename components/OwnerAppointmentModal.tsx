      ...
      {step === 2 && <div className="space-y-5 p-5 sm:p-8">
        <div className="grid gap-4 sm:grid-cols-3">
          <label className="block text-sm font-medium">Category
            <select value={categoryId || ""} onChange={event => { setCategoryId(Number(event.target.value) || null); setSubcategoryId(null); setServiceId(null); setLengthOptionId(null); setFoundation(""); setTexture(""); }} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4">
              <option value="">Choose a category</option>
              {categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}
            </select>
          </label>
          <label className="block text-sm font-medium">Subcategory / style
            <select value={subcategoryId || ""} disabled={!selectedCategory} onChange={event => { const value = event.target.value; setSubcategoryId(value === "direct" ? "direct" : Number(value) || null); setServiceId(null); setLengthOptionId(null); setFoundation(""); setTexture(""); }} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4 disabled:bg-neutral-100">
              <option value="">Choose a subcategory</option>
              {styleChoices.map(style => <option key={style.id} value={style.id}>{style.name}</option>)}
            </select>
          </label>
          <label className="block text-sm font-medium">Service / size
            <select value={serviceId || ""} disabled={!selectedSubcategory} onChange={event => { setServiceId(Number(event.target.value) || null); setLengthOptionId(null); setFoundation(""); setTexture(""); }} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4 disabled:bg-neutral-100">
              <option value="">Choose a service</option>
              {services.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}
            </select>
          </label>
        </div>
        {selectedService?.lengthOptions?.length ? <label className="block text-sm font-medium">Length<select value={lengthOptionId || ""} onChange={event => setLengthOptionId(Number(event.target.value) || null)} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4"><option value="">Choose a length</option>{selectedService.lengthOptions.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label> : null}
        {selectedService?.foundationChoicesEnabled && <label className="block text-sm font-medium">Foundation<select value={foundation} onChange={event => setFoundation(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4"><option value="">Choose a foundation</option><option value="REGULAR">Regular</option><option value="KNOTLESS">Knotless</option></select></label>}
        {selectedService?.hairTextures?.length ? <label className="block text-sm font-medium">Texture<select value={texture} onChange={event => setTexture(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] bg-white px-4"><option value="">Choose a texture</option>{selectedService.hairTextures.map(value => <option key={value}>{value}</option>)}</select></label> : null}
        {availableAddOns.length > 0 && <fieldset><legend className="text-sm font-medium">Add-ons</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{availableAddOns.map(addOn => <label key={addOn.id} className="flex items-center justify-between rounded-xl border bg-white p-4"><span><b className="block text-sm">{addOn.name}</b><small>{addOn.pricingMode === "STARTING_AT" ? "From " : "+"}{money(addOn.priceCents)}</small></span><input type="checkbox" checked={addOnIds.includes(addOn.id)} onChange={event => setAddOnIds(ids => event.target.checked ? [...ids, addOn.id] : ids.filter(id => id !== addOn.id))}/></label>)}</div></fieldset>}
        {quote && <div className="rounded-xl border border-[#e2d4c5] bg-[#faf7f3] p-5"><p className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#8b4e31]">{selectedCategory?.name} → {selectedSubcategory?.name} → {selectedService?.name}</p><div className="flex justify-between"><span>Service total</span><b>{money(quote.servicePriceCents)}</b></div><div className="mt-2 flex justify-between text-sm"><span>Configured deposit</span><b>{money(quote.depositCents)}</b></div></div>}
