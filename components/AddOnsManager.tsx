...
import { EllipsisVertical, Library, Loader2, Pencil, Plus, Search, X } from "lucide-react";
...
export function AddOnsManager({ sub, items, data, token, onError, onSuccess, embeddedMobile = false }: Props) {
  ...
  return <>
    <section className={`w-full overflow-hidden bg-white dark:bg-neutral-900 ${embeddedMobile ? "border-0 shadow-none" : "rounded-xl border border-neutral-200 shadow-sm dark:border-neutral-700"}`}>
      <div className={`${embeddedMobile ? "hidden" : "flex"} items-center justify-between gap-4 border-b border-neutral-200 px-5 py-4 dark:border-neutral-800 sm:px-6`}>
